 ...
 * @param outputs - An array of outputs corresponding to each condition. should be the same length as conditions.
...
export function selectArrayTernary<T>(conditions: boolean[], outputs: T[], defaultValue: T): T {
  ...
  if (conditions.length !== outputs.length) {
    throw new Error('selectArrayTernary: conditions and outputs arrays must be the same length');
  }
